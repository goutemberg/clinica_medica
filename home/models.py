...
class CadastroEmpresa(models.Model):
    ...
    telefone = models.CharField(max_length=16, null=True , validators=[
        RegexValidator(regex='^\(?\d{2}\)?[\s-]?\d{4,5}-?\d{4}$', message='Telefone deve estar no formato (XX) XXXX-XXXX ou (XX) XXXXX-XXXX')
    ])
    celular = models.CharField(max_length=16, default='00000000000', null=True, validators=[
        RegexValidator(regex='^\(?\d{2}\)?[\s-]?\d{4,5}-?\d{4}$', message='Celular deve estar no formato (XX) XXXXX-XXXX')
    ])  # Adicionado valor padrão
    pessoa_contato = models.CharField(max_length=50, null=True)
    email = models.EmailField(max_length=50, validators=[EmailValidator(message='Email inválido')], null=True)
    isento_tributacao = models.BooleanField(default=False)
    taxa_administrativa = models.DecimalField(max_digits=6, decimal_places=2, default=00)
    #valor_imposto = models.CharField(max_length=5, null=True)
...
class CadastroMedico(models.Model):
    ...
    telefone2 = models.CharField(max_length=16, null=True)
...
class Plantao(models.Model):
    ...
    contato_emergencia = models.CharField(max_length=255, null=True)
    equipamentos_necessarios = models.TextField(null=True)
    cargos_auxiliares = models.TextField(null=True)
    substituto = models.CharField(max_length=255, null=True)
